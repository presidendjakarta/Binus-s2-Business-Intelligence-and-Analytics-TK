...
3. [Deep-Dive Analisis Sentimen pada 4 Pilar Aspek Operasional](#3-deep-dive-analisis-sentimen-pada-4-pilar-aspek-operasional)
   ...
   - [3.2. Aspek 2: Transaksi Online & Integrasi Fasilitas Kesehatan (Pembayaran)](#32-aspek-2-Transaksi-online--integrasi-fasilitas-kesehatan-Pembayaran)
   - [3.3. Aspek 3: Kinerja, Stabilitas Aplikasi & Server](#33-aspek-3-kinerja-stabilitas-aplikasi--server)
   - [3.4. Aspek 4: Pembayaran Layanan & Fitur Finansial Kartu Digital](#34-aspek-4-pembayaran-Tagihan--layanan-kartu-digital)
...
Aplikasi **Livin' by Mandiri** merupakan ujung tombak digitalisasi program Jaminan Kesehatan Nasional (JKN) yang dikelola oleh PT Bank Mandiri (Persero) Tbk. Dari ekstraksi **5.000 ulasan publik Google Play Store**, sistem berhasil mengidentifikasi pola kepuasan dan titik kritis friksi (*pain points*) pengguna secara objektif.

```mermaid
pie title Distribusi Sentimen Pengguna Livin' by Mandiri (N = 5.000)
...
2. **Titik Friksi Terbesar**: Hambatan terbesar berpusat pada **Sistem Transaksi Online Pembayaran ($37.0\%$)** dan **Autentikasi Akun / OTP ($28.4\%$)**.
...
Seluruh ulasan dipetakan secara otomatis ke dalam 4 pilar fungsional Livin' by Mandiri menggunakan fungsi [`detectAspects`](file:///x:/laragon/kuliah/playstore-mining/run-analisa.js#L13-L34):
...
| **Transaksi & Pembayaran** | 1.850 | 610 (33.0%) | 1.240 (67.0%) | 33.0% | 🔴 Kritis (Kuota & SIMRS) |
| **Kinerja & Server** | 1.290 | 180 (14.0%) | 1.110 (86.0%) | 14.0% | 🔴 Sangat Kritis (Crash / RTO) |
| **Layanan & Fitur Finansial** | 940 | 520 (55.3%) | 420 (44.7%) | 55.3% | 🟢 Positif (KIS Digital) |
...
2. **Konflik NIK & Nomor HP Ganda**: Anggota keluarga yang terdaftar dalam satu Kartu Keluarga (KK) kesulitan mendaftar akun terpisah karena validasi sistem mengunci 1 nomor HP per kenasabahan.
...
### 3.2. Aspek 2: Transaksi Online & Integrasi Fasilitas Kesehatan (Pembayaran)
- **Volume Ulasan**: $1.850$ ulasan ($37.0\%$ total data - **Aspek Terbesar**).
- **Rasio Sentimen**: **$33.0\%$ Positif** vs **$67.0\%$ Negatif**.

#### 🚨 Akar Masalah Utama (Root Causes):
1. **Kuota Transaksi Cepat Habis Secara Misterius**: Kuota pendaftaran online cabang/klinik sering kali sudah berstatus *"Penuh"* dalam hitungan detik setelah loket dibuka (pukul 00.00 atau 06.00 WIB).
2. **Disinkronisasi Jadwal Dokter SIMRS**: Pasien yang telah mengambil nomor Transaksi online di Livin' by Mandiri sering mendapati dokter yang bersangkutan cuti/tidak praktik saat tiba di kantor cabang rujukan.
3. **Keterlambatan Pembaruan Status Rujukan**: Surat rujukan berjenjang dari cabang ke FKRTL tidak langsung muncul di aplikasi saat pasien hendak berobat.
...
2. **Request Time Out (RTO) pada Jam Sibuk**: Server mengalami penurunan performa drastis (*bottleneck*) pada rentang waktu sibuk pendaftaran Pembayaran (pukul 07.00 – 09.30 WIB).
3. **Looping Loading Screen**: Aplikasi mengalami animasi *loading* tanpa akhir saat mengakses menu riwayat pelayanan.

---

### 3.4. Aspek 4: Pembayaran Layanan & Fitur Finansial Kartu Digital
...
- **Keterlambatan Pembaruan Status Pembayaran**: Pengguna telah membayar Tagihan lewat m-banking/minimarket, namun status kenasabahan di Livin' by Mandiri masih tercatat *"Menunggak / Non-Aktif"* hingga 1x24 jam.
...
2. **Pemulihan Sentimen pada Agustus 2026 ($42.5\%$)**: Rilis patch `v4.18.0` berhasil memperbaiki isu *crash*, namun kendala kuota Transaksi Pembayaran masih menjadi isu dominan yang belum terselesaikan.
...
| 3 | `bagus` | 198.7 | `tidak_masuk` (OTP/Tagihan) | 275.8 |
| 4 | `cepat` | 182.4 | `lambat` (lemot/lola) | 242.1 |
| 5 | `mantap` | 165.9 | `antre` (Transaksi habis) | 228.6 |
...
Berdasarkan analisis akar masalah dan bobot kata kunci, disusun **Matriks Prioritas Solusi (Dampak Bisnis vs Kemudahan Implementasi)** untuk jajaran manajemen PT Bank Mandiri (Persero) Tbk:
...
│                     │ 6. Realtime Sync SIMRS   │ 8. Audit Kuota Pembayaran       │
└─────────────────────┴──────────────────────────┴─────────────────────────────┘
```
...
- **Offline Caching Kartu Digital**: Mengaktifkan penyimpanan lokal (*offline storage*) untuk Kartu KIS Digital agar kartu tetap dapat ditampilkan pasien di kantor cabang meskipun server sedang mengalami gangguan jaringan.

#### 2. Jangka Menengah (3 – 6 Bulan) - *Major Technical Projects*:
- **Elastic Cloud Auto-Scaling**: Menerapkan arsitektur *auto-scaling* pada kluster server API Livin' by Mandiri untuk mengantisipasi lonjakan beban (*traffic spike*) pada jam sibuk (06.30 – 09.30 WIB).
- **Standarisasi Integrasi API SIMRS**: Menerapkan protokol komunikasi dua arah secara *real-time* dengan Sistem Informasi Manajemen kantor cabang (SIMRS) guna menjamin jadwal praktik dokter selalu akurat.

#### 3. Jangka Panjang (6 – 12 Bulan) - *Governance & Policy Improvement*:
- **Audit Transparansi Kuota Pembayaran**: Membangun dashboard pemantauan alokasi kuota Transaksi online vs Transaksi *walk-in* di tingkat cabang untuk mencegah penutupan kuota sepihak oleh oknum fasilitas kesehatan.
- **Asisten Virtual Cerdas (AI Virtual Care)**: Menghadirkan chatbot berbasis NLP untuk memandu nasabah menyelesaikan kendala lupa sandi dan perubahan data tanpa harus datang ke kantor cabang.

---
*Dokumen ini merupakan analisis bisnis strategis dan rekomendasi manajerial resmi proyek Livin' by Mandiri Sentiment Analytics.*
